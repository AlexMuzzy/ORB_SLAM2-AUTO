...
namespace ORB_SLAM2 {
    ...
    cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp, bool& isKeyFrame) {
        if (mSensor != MONOCULAR) {
        ...
        cv::Mat Tcw = mpTracker->GrabImageMonocular(im, timestamp, isKeyFrame);
    ...
    void System::SaveTrajectoryTUM(const string &filename) {
        ...
        for (list<Tracking::TrackedFrame>::iterator iter = mpTracker->tracked_frames.begin(), iter_end = mpTracker->tracked_frames.end();
             iter != iter_end; iter++) {
            if (iter->lost)
                continue;
            ...
            f << setprecision(6) << iter->time << " " << setprecision(9) << twc.at<float>(0) << " " << twc.at<float>(1)
